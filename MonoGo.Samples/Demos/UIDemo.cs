...
namespace MonoGo.Samples.Demos
{
    public class UIDemo : GUIEntity
    {
        ...
        public override void CreateUI()
        {
            ...
            Panel topPanel = new(GUIMgr.System, null!)
            {
            ...
            // add button to enable debug mode
            {
                ...
                button.Events.OnClick = (Entity Entity) => GUIMgr.DebugDraw = !GUIMgr.DebugDraw;
                topPanel.AddChild(button);
            }

            // add button to the GitHub repo
            {
                Button button = new(GUIMgr.System, "GitHub")
                {
                    Anchor = Anchor.TopCenter
                };
                button.Size.SetPixels(240, topPanelHeight);
            ...
            DropDown themeDropDown = new(GUIMgr.System, listPanelCentered, listItemCentered)
            {
            ...
            themeDropDown.Events.OnValueChanged = (Entity Entity) =>
            {
            ...
            _nextExampleButton = new(GUIMgr.System, "GUI.Next ->")
            {
                Anchor = Anchor.TopCenter
            };
            ...
            _nextExampleButton.Events.OnClick = (Entity Entity) => NextExample();
            ...
            if (true)
            {
                // example: welcome message
                {
                    ...
                    var welcomeText = new Paragraph(GUIMgr.System, @$"Welcome to ${{FC:FFDB5F}}Mono${{FC:e60000}}Go${{RESET}}!

A Cross-Platform C# 2D game engine build ontop of ${{FC:e60000}}MonoGame${{RESET}}.
...
") { TextOverflowMode = TextOverflowMode.WrapWords };
                    ...
                    var version = new Paragraph(GUIMgr.System, "${FC:FFDB5F}v" + Assembly.GetAssembly(typeof(Engine.EC.Entity)).GetName().Version + "${RESET}")
                    {
                ...
                {
                    // create panel
                    var panel = new Panel(GUIMgr.System);
                    ...
                    if (demoTitle != null)
                    {
                        ...
                        panel.AddChild(new Title(GUIMgr.System, demoTitle) { Anchor = Anchor.AutoCenter });
                        panel.AddChild(new HorizontalLine(GUIMgr.System));
                    }
                ...
                // anchors
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Entitys are positioned using Anchors. An Anchor can be a pre-defined position on the parent Entity, like Top-Left, or Center."));

                    var anchorsPanel = new Panel(GUIMgr.System);
                    ...
                    anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "TopLeft") { Anchor = Anchor.TopLeft });
                    anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "TopRight") { Anchor = Anchor.TopRight });
                    anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "TopCenter") { Anchor = Anchor.TopCenter });
                    anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "BottomLeft") { Anchor = Anchor.BottomLeft });
                    anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "BottomRight") { Anchor = Anchor.BottomRight });
                    anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "BottomCenter") { Anchor = Anchor.BottomCenter });
                    anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "CenterLeft") { Anchor = Anchor.CenterLeft });
                    anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "CenterRight") { Anchor = Anchor.CenterRight });
                    anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "Center") { Anchor = Anchor.Center });
                }

                // auto anchors
                {
                    var panel = CreateDemoContainer("Auto Anchors", new Point(750, 1));
                    panel.AddChild(new Paragraph(GUIMgr.System,
                        @"Previously we saw regular Anchors. Now its time to explore the Automatic anchors."));

                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    var anchorsPanel = new Panel(GUIMgr.System);
                    anchorsPanel.Size.X.SetPercents(100f);
                    ...
                    {
                        anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "AutoLTR first item.") { Anchor = Anchor.AutoLTR });
                        anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "AutoLTR second item. Will be in a different row.") { Anchor = Anchor.AutoLTR });
                        var btn = anchorsPanel.AddChild(new Button(GUIMgr.System, "Button set to AutoLTR too.") { Anchor = Anchor.AutoLTR });
                        btn.Size.X.SetPixels(400);
                    }
                    anchorsPanel.AddChild(new HorizontalLine(GUIMgr.System));
                    {
                        anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "This item is AutoRTL.") { Anchor = Anchor.AutoRTL });
                        anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "AutoRTL second item. Will be in a different row.") { Anchor = Anchor.AutoRTL });
                        var btn = anchorsPanel.AddChild(new Button(GUIMgr.System, "Button set to AutoRTL too.") { Anchor = Anchor.AutoRTL });
                        btn.Size.X.SetPixels(400);
                    }
                    anchorsPanel.AddChild(new HorizontalLine(GUIMgr.System));
                    {
                        {
                            anchorsPanel.AddChild(new Paragraph(GUIMgr.System, "We also have inline anchors that arrange entities next to each other, and only break line when need to. For example, AutoInlineLTR buttons:") { Anchor = Anchor.AutoLTR });
                            for (int i = 0; i < 5; ++i)
                            {
                                var btn = anchorsPanel.AddChild(new Button(GUIMgr.System, "AutoInlineLTR") { Anchor = Anchor.AutoInlineLTR });
                ...
                // panels
                {
                    ...
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    {
                        var panelLeft = new Panel(GUIMgr.System, null!);
                        ...
                        panelLeft.AddChild(new Paragraph(GUIMgr.System, "Left Panel"));
                        panelLeft.AddChild(new Button(GUIMgr.System));
                    }
                    {
                        var panelRight = new Panel(GUIMgr.System, null!);
                        ...
                        panelRight.AddChild(new Paragraph(GUIMgr.System, "Right Panel"));
                        panelRight.AddChild(new Button(GUIMgr.System));
                    }

                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"You can add a small title to panels when you create them:"));
                    panel.AddChild(new RowsSpacer(GUIMgr.System, 2));
                    {
                        var titledPanel = new Panel(GUIMgr.System);
                        ...
                        var title = new Paragraph(GUIMgr.System, panelTitleStyle, "Panel Title");
                        titledPanel.AddChild(title);
                        title.Anchor = Anchor.TopCenter;
                        title.Offset.Y.SetPixels(-26);

                        titledPanel.AddChild(new Paragraph(GUIMgr.System, "Looks nice, isn't it? Check out the source code to see how we did it."));
                    }
...
The small box in the corner is draggable too:"));
                    ...
                    var draggableBox = panel.AddChild(new Panel(GUIMgr.System)
                    {
                ...
                // buttons
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, "Easily place buttons and register click events:"));
                    {
                        int clicksCount = 0;
                        var btn = panel.AddChild(new Button(GUIMgr.System, "Click Me!"));
                    ...
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Buttons can also function as checkboxes, allowing you to click on them to toggle their state (checked/unchecked):"));
                    {
                        var btn = panel.AddChild(new Button(GUIMgr.System, "Toggle Me!"));
                        btn.ToggleCheckOnClick = true;
                    }

                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"And they can even function as a radio button, meaning only one button can be checked at any given time:"));
                    {
                        var btn = panel.AddChild(new Button(GUIMgr.System, "First Option"));
                        btn.ToggleCheckOnClick = true;
                        btn.CanClickToUncheck = false;
                        btn.ExclusiveSelection = true;
                    }
                    {
                        var btn = panel.AddChild(new Button(GUIMgr.System, "Second Option"));
                        btn.ToggleCheckOnClick = true;
                        btn.CanClickToUncheck = false;
                        btn.ExclusiveSelection = true;
                    }
                    {
                        var btn = panel.AddChild(new Button(GUIMgr.System, "Third Option"));
                        btn.ToggleCheckOnClick = true;
                        btn.CanClickToUncheck = false;
                        btn.ExclusiveSelection = true;
                    }
...
And you can even embed icons {smileyIcon} inside text paragraphs!"));
                ...
                // checkbox and radio
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Basic Checkbox Entity:"));
                    panel.AddChild(new Checkbox(GUIMgr.System, "Checkbox Option 1"));
                    panel.AddChild(new Checkbox(GUIMgr.System, "Checkbox Option 2"));
                    panel.AddChild(new Checkbox(GUIMgr.System, "Checkbox Option 3"));

                    panel.AddChild(new HorizontalLine(GUIMgr.System));

                    panel.AddChild(new Paragraph(GUIMgr.System, @"Radio button Entitys:"));
                    panel.AddChild(new RadioButton(GUIMgr.System, "Radio Option 1")).Checked = true;
                    panel.AddChild(new RadioButton(GUIMgr.System, "Radio Option 2"));
                    panel.AddChild(new RadioButton(GUIMgr.System, "Radio Option 3"));
                }
                ...
                // sliders
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Sliders are useful to select numeric values:"));
                    {
                        var slider = panel.AddChild(new Slider(GUIMgr.System));
                        var label = panel.AddChild(new Label(GUIMgr.System, @$"Slider Value: {slider.Value}"));
                        panel.AddChild(new RowsSpacer(GUIMgr.System, 2));
                        slider.Events.OnValueChanged = (Entity Entity) => { label.Text = $"Slider Value: {slider.Value}"; };
                    }

                    panel.AddChild(new Paragraph(GUIMgr.System, @"Sliders can also be vertical:"));
                    {
                        var slider = panel.AddChild(new Slider(GUIMgr.System, Orientation.Vertical));
                ...
                // progress bars
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Progress Bars are similar to sliders, but are designed to show progress or things like health bars:"));
                    {
                        var progressBar = panel.AddChild(new ProgressBar(GUIMgr.System));
                        var label = panel.AddChild(new Label(GUIMgr.System, @$"Progress Bar Value: {progressBar.Value}"));
                        panel.AddChild(new RowsSpacer(GUIMgr.System));
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"By default Progress Bars are not interactable, but you can make them behave like sliders by settings 'IgnoreInteractions' to false:"));
                    {
                        var progressBar = panel.AddChild(new ProgressBar(GUIMgr.System));
                        var label = panel.AddChild(new Label(GUIMgr.System, @$"Progress Bar Value: {progressBar.Value}"));
                        panel.AddChild(new RowsSpacer(GUIMgr.System));
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"And finally, here's an alternative progress bar design, without animation:"));
                    {
                        var progressBar = panel.AddChild(new ProgressBar(GUIMgr.System, hProgressBarAltStyle, hProgressBarAltFillStyle));
                ...
                // list box
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"List Boxes allow you to add items and select them from a list:"));
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    {
                        panel.AddChild(new Label(GUIMgr.System, @"Select Race:"));
                        var listbox = panel.AddChild(new ListBox(GUIMgr.System));
                    ...
                    {
                        panel.AddChild(new Paragraph(GUIMgr.System, @"Clear the selection by clicking the selected item again."));
                        panel.AddChild(new RowsSpacer(GUIMgr.System));
                        panel.AddChild(new Label(GUIMgr.System, @"Select Class:"));
                        var listbox = panel.AddChild(new ListBox(GUIMgr.System));
                        ...
                        var selectedParagraph = panel.AddChild(new Paragraph(GUIMgr.System));
                ...
                // drop down
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Drop Down entities are basically list boxes, but the list is hidden while not interacted with. For example:"));
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    {
                        panel.AddChild(new Label(GUIMgr.System, @"Select Race:"));
                        var dropdown = panel.AddChild(new DropDown(GUIMgr.System));
                    ...
                    {
                        panel.AddChild(new Paragraph(GUIMgr.System, @"In the dropdown below, you can clear selection by clicking the selected item again."));
                        panel.AddChild(new RowsSpacer(GUIMgr.System));
                        panel.AddChild(new Label(GUIMgr.System, @"Select Class:"));
                        var dropdown = panel.AddChild(new DropDown(GUIMgr.System));
                ...
                // color inputs
                {
                    ...
                    // color slider
                    {
                        panel.AddChild(new Paragraph(GUIMgr.System, @"Color Slider entities can be used to get a color value from a range using a slider and a source texture:"));
                        var slider = panel.AddChild(new ColorSlider(GUIMgr.System));
                        var value = panel.AddChild(new Label(GUIMgr.System));
                        ...
                        {
                            var colorBtn = panel.AddChild(new Button(GUIMgr.System, "Red"));
                        ...
                        {
                            var colorBtn = panel.AddChild(new Button(GUIMgr.System, "Green"));
                        ...
                        {
                            var colorBtn = panel.AddChild(new Button(GUIMgr.System, "Blue"));
                            colorBtn.Anchor = Anchor.AutoInlineLTR;
                            colorBtn.Size.X.SetPercents(24f);
                            colorBtn.Events.OnClick = (Entity Entity) =>
                            {
                        ...
                        {
                            var colorBtn = panel.AddChild(new Button(GUIMgr.System, "Purple"));
                            colorBtn.Anchor = Anchor.AutoInlineLTR;
                            colorBtn.Size.X.SetPercents(24f);
                            colorBtn.Events.OnClick = (Entity Entity) =>
                            {
                    ...
                    // color picker
                    {
                        panel.AddChild(new Paragraph(GUIMgr.System, @"Color Picker entities can be used to get a color value from a rectangle region by picking pixels off a source texture:"));
                        var picker = panel.AddChild(new ColorPicker(GUIMgr.System));
                        var value = panel.AddChild(new Label(GUIMgr.System));
...
This panel has some random Entitys below that go wayyyy down.

Use the scrollbar on the right to see more of it.
"));
                    panel.AddChild(new Button(GUIMgr.System, "Some Button"));
                    panel.AddChild(new Paragraph(GUIMgr.System,
                        @"
Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.
"));
                    panel.AddChild(new Button(GUIMgr.System, "Another Button"));
                    panel.AddChild(new Slider(GUIMgr.System));
                    panel.AddChild(new Checkbox(GUIMgr.System, "A Checkbox"));
                    panel.AddChild(new RadioButton(GUIMgr.System, "A Radio Button"));
                    var listbox = panel.AddChild(new ListBox(GUIMgr.System));
                    listbox.AddItem("Human");
                    listbox.AddItem("Elf");
                    listbox.AddItem("Orc");
                ...
                // text input
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Text Input entity is useful to get free text input from users. This is a single-line text input:"));
                    {
                        var textInput = panel.AddChild(new TextInput(GUIMgr.System));
                        textInput.PlaceholderText = "Click to edit text input.";
                    }

                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"And here's a multiline text input:"));
                    {
                        var textInput = panel.AddChild(new TextInput(GUIMgr.System));
                    ...
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"You can also mask the text, for password input:"));
                    {
                        var textInput = panel.AddChild(new TextInput(GUIMgr.System));
                        textInput.PlaceholderText = "Password";
                        textInput.MaskingCharacter = '*';
                        var showPassword = panel.AddChild(new Checkbox(GUIMgr.System, "Show Password"));
                ...
                // numeric text input
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Numeric text input get float or integer value from the user in a form similar to a text input. For example, with decimal point:"));
                    {
                        var textInput = panel.AddChild(new NumericInput(GUIMgr.System));
                    }

                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"This Numeric Input don't accept a decimal point:"));
                    {
                        var textInput = panel.AddChild(new NumericInput(GUIMgr.System));
                    ...
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"This Numeric Input has min and max limits (-10, 10):"));
                    {
                        var textInput = panel.AddChild(new NumericInput(GUIMgr.System));
                    ...
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"You can also create a Numeric Input entity without the buttons:"));
                    {
                        var textInput = panel.AddChild(new NumericInput(GUIMgr.System, false, false));
                    }
                }
                ...
                // message boxes
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Message boxes are useful to get quick input from the user. Iguina comes with a utility to generate common message boxes.

Click below to see some examples."
                    ));

                    panel.AddChild(new Button(GUIMgr.System, "Show Info Message Box")).Events.OnClick = (Entity entity) =>
                    {
                        GUIMgr.System.MessageBoxes.ShowInfoMessageBox("Info Message Box",
...
You can just close it."
                    ...
                    panel.AddChild(new Button(GUIMgr.System, "Show Confirmation Message Box")).Events.OnClick = (Entity entity) =>
                    {
                        GUIMgr.System.MessageBoxes.ShowConfirmMessageBox("Confirmation Message Box",
...
You can just close it."
                    ...
                    panel.AddChild(new Button(GUIMgr.System, "Show Save File Dialog")).Events.OnClick = (Entity entity) =>
                    {
                        GUIMgr.System.MessageBoxes.ShowSaveFileDialog("Save File Dialog",
                            @"This is an interactive save file dialog box.
Don't worry, nothing will actually be saved / written, feel free to play around.",
                            (string filename) =>
                            {
                                GUIMgr.System.MessageBoxes.ShowInfoMessageBox("Success!", $"Selected file: '{filename}'.");
                                return true;
                            });
                    };

                    panel.AddChild(new Button(GUIMgr.System, "Show Open File Dialog")).Events.OnClick = (Entity entity) =>
                    {
                        GUIMgr.System.MessageBoxes.ShowOpenFileDialog("Open File Dialog",
...
Don't worry, nothing will actually be opened, feel free to play around.",
                ...
                // locked / disabled
                {
                    ...
                    panel.AddChild(new Paragraph(GUIMgr.System, @"You can disable Entitys to make them ignore user interactions and render them with 'disabled' effect (you can create your own effects for this):"));
                    panel.AddChild(new Button(GUIMgr.System, "Disabled Button") { Enabled = false });
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"When you disable a panel, all Entitys under it will be disabled too.
...
If you want to just lock items without rendering them with 'disabled' style, you can also set the Locked property. For example the following button is locked, but will render normally:"));
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Button(GUIMgr.System, "Locked Button") { Locked = true });
                    panel.AddChild(new RowsSpacer(GUIMgr.System));
                    panel.AddChild(new Paragraph(GUIMgr.System, @"Any type of Entity can be locked and disabled:"));
                    panel.AddChild(new Slider(GUIMgr.System) { Enabled = false });
                    panel.AddChild(new Checkbox(GUIMgr.System, "Disabled Checkbox") { Enabled = false });
                    panel.AddChild(new RadioButton(GUIMgr.System, "Disabled Radio Button") { Enabled = false });
                    var listbox = panel.AddChild(new ListBox(GUIMgr.System));
                    listbox.AddItem("Human");
                    listbox.AddItem("Elf");
                    listbox.AddItem("Orc");
                ...
                // example: epilogue
                {
                    ...
                    // add title and text
                    panel.AddChild(new Paragraph(GUIMgr.System, $@"That was only the GUI-DEMO! There is still much to learn about ${{FC:FFDB5F}}Mono${{FC:e60000}}Go${{RESET}}.
